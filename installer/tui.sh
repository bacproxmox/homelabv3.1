...
export HOMELAB_VERSION="${HOMELAB_VERSION:-3.1}"
...
ensure_session() {
  ...
  if [[ -z "$SESSION_DIR" || ! -d "$SESSION_DIR" ]]; then
    SESSION_DIR="$LOG_DIR/v3.1-session-$(date +%Y%m%d-%H%M%S)"
    mkdir -p "$SESSION_DIR"
    echo "$SESSION_DIR" > "$SESSION_FILE"
    chmod 600 "$SESSION_FILE" 2>/dev/null || true
  fi
  MASTER_LOG="$SESSION_DIR/00-v3.1-master.log"
  touch "$MASTER_LOG"
...
render_progress_dashboard() {
  ...
  clear || true
  echo "============================================================"
  echo " Homelab v3.1 Modular Guided Install"
  echo "============================================================"
  echo
  ...
  echo
  echo "Not: TrueNAS manuel kurulum/checkpoint disinda guided akista adim arasi onay beklenmez."
  echo "------------------------------------------------------------"
}

show_progress() {
  ensure_session
  ui_msg "Homelab v3.1 ilerleme" "$(progress_text)" 36 100
}
...
run_step_index() {
  ...
  if [[ "$mode" == "guided" ]]; then
    ...
    ui_msg "Homelab v3.1 - Adim $((idx + 1))/${#STEP_IDS[@]}" "Genel ilerleme: ${percent}%\n[$bar]\n\nSimdi calisacak adim:\n$title\n\nTarget:\n$target\n\nLog:\n$step_log" 22 92
  fi
...
collect_support_bundle() {
  ...
  set +e
  homelab_run "maintenance/logs/collect-support-bundle.sh" 2>&1 | tee "$support_log"
...
main_menu() {
  ...
  while true; do
    local choice
    choice="$(ui_menu "Homelab v3.1 Modular TUI" "Secim yap." 20 88 10 \
